...
const size_t seq_threshold_for_lce = 1025;

template<typename F = flag_type<false>, typename index_type, typename buffer_type>
inline void phase_2_by_sorting_stable_parallel_for_lce(sa_type<index_type> &sa, buffer_type *const isa,
  ...
  for (count_type g = 2; g < number_of_groups; ++g) {
    ...
    else if (gsize < seq_threshold_for_lce) {
